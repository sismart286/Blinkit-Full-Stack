...
const ProductListPage = () => {
  ...
  const categoryId = params.category.split("-").slice(-1)[0]; //here "slice(-1) means the last elemet"
  const subCategoryId = params.subCategory.split("-").slice(-1)[0]; //here "slice(-1) means the last elemet"
        ...
        <div className="min-h-[88vh] max-h-[88vh] overflow-y-scroll grid gap-1 shadow-md scrollbarCustom bg-white py-2">
          {DisplaySubCategory.map((s, index) => {
            ...
            return (
              <Link
                to={link}
                className={`w-full p-2 lg:flex items-center lg:w-full lg:h-16 box-border lg:gap-4 border-b border-blue-100 hover:bg-green-300 cursor-pointer ${
              ...
              >
                <div className="w-fit max-w-28 mx-auto lg:mx-0 bg-white rounded box-border">
                  <img
                    src={s.image}
                    alt="subCategory"
                    className="w-15 lg:h-14 lg:w-12 h-full object-scale-down"
                  />
                </div>
